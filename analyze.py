...
for day_file in ['prices_round_0_day_-2.csv', 'prices_round_0_day_-1.csv']:
    ...
    for product in sorted(products):
        ...
        if mid_prices:
            print(f"  Mid Price - Min: {min(mid_prices):.2f}, Max: {max(mid_prices):.2f}, "
                  f"Mean: {sum(mid_prices)/len(mid_prices):.2f}")
            print(f"  Best Bid - Min: {min(bid_prices):.2f}, Max: {max(bid_prices):.2f}, "
                  f"Mean: {sum(bid_prices)/len(bid_prices):.2f}")
            print(f"  Best Ask - Min: {min(ask_prices):.2f}, Max: {max(ask_prices):.2f}, "
                  f"Mean: {sum(ask_prices)/len(ask_prices):.2f}")
            print(f"  Spread   - Min: {min(spreads):.2f}, Max: {max(spreads):.2f}, "
                  f"Mean: {sum(spreads)/len(spreads):.2f}")
            
            # Volatility
            returns = [(mid_prices[i] - mid_prices[i-1]) for i in range(1, len(mid_prices))]
            if returns:
            ...
            try:
                bid_vol_1 = [abs(float(r.get('bid_volume_1', '0') or '0')) for r in prod_rows]
                ask_vol_1 = [abs(float(r.get('ask_volume_1', '0') or '0')) for r in prod_rows]
            ...
            try:
                ...
                if any(p != 0 for p in pnls):
                    print(f"  PnL - Min: {min(pnls):.2f}, Max: {max(pnls):.2f}, Final: {pnls[-1]:.2f}")
            except:
                pass

# Print sample row to see all columns
print(f"\n\nSample row (first price entry):")
sample_rows = load_prices('prices_round_0_day_-1.csv')
...
for day_file in ['trades_round_0_day_-2.csv', 'trades_round_0_day_-1.csv']:
    ...
    for product in sorted(products):
        ...
        print(f"  Price - Min: {min(prices):.2f}, Max: {max(prices):.2f}, Mean: {sum(prices)/len(prices):.2f}")
        print(f"  Qty - Min: {min(quantities):.2f}, Max: {max(quantities):.2f}, Mean: {sum(quantities)/len(quantities):.2f}")
...
rows = load_prices('prices_round_0_day_-1.csv')
emerald_rows = [r for r in rows if r.get('product') == 'RAINFOREST_RESIN']
if not emerald_rows:
    emerald_rows = [r for r in rows if 'EMERALD' in r.get('product', '').upper() or 'RESIN' in r.get('product', '').upper()]
...
for product in all_products:
    ...
    if mid_prices:
        ...
        for price, count in sorted(rounded.items(), key=lambda x: -x[1])[:10]:
            print(f"    {price}: {count} occurrences ({count/len(mid_prices)*100:.1f}%)")
