"""
Round 1 Backtester — adapted from backtest.py
- Imports round1.py (not trader.py)
- Position limit = 80 (not 20)
- Uses historical_data/ Round 1 CSVs
"""
...
spec = importlib.util.spec_from_file_location("trader_module", "round1.py")
...
def run_backtest(price_file, log_file=None):
    ...
    for ts in sorted(snapshots.keys()):
        ...
        for product, prod_orders in result_orders.items():
            ...
            # Check aggregate order validity (position limit enforcement)
        ...
        if iteration_count % 100 == 0 or iteration_count <= 5:
            pos_str = ", ".join(f"{p}={v}" for p, v in sorted(positions.items()))
            print(f"  t={ts:6d} | PnL={running_pnl:10.1f} | {pos_str}")
...
# ---- Run on all Round 1 data ----
total = 0
data_dir = "historical_data"

files = sorted([
    os.path.join(data_dir, f) for f in os.listdir(data_dir)
    if f.startswith("prices_round_1") and f.endswith(".csv")
])

if not files:
    print("ERROR: No Round 1 price files found in historical_data/")
    sys.exit(1)
