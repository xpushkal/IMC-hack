...
for day_file in ["Data/prices_round_0_day_-1.csv"]:
    ...
    for product in ["EMERALDS", "TOMATOES"]:
        ...
        for level in [1, 2, 3]:
            ...
            if bids:
                print(f"\n  Level {level}:")
                print(
                    f"    Bid - Mean: {sum(bids) / len(bids):.2f}, Count: {len(bids)}"
                )
                print(
                    f"    Ask - Mean: {sum(asks) / len(asks):.2f}, Count: {len(asks)}"
                )
        ...
        if product == "EMERALDS":
            ...
            for p in sorted(bid_dist.keys()):
                print(
                    f"    {p}: {bid_dist[p]} ({bid_dist[p] / len(all_bids_l1) * 100:.1f}%)"
                )

            print(f"  Ask price distribution:")
            for p in sorted(ask_dist.keys()):
                print(
                    f"    {p}: {ask_dist[p]} ({ask_dist[p] / len(all_asks_l1) * 100:.1f}%)"
                )
        ...
        if product == "TOMATOES":
            ...
            for sp in sorted(spread_dist.keys()):
                ...
                print(
                    f"    Spread={sp}: {spread_dist[sp]} ({spread_dist[sp] / total * 100:.1f}%)"
                )
...
for day_file in ["Data/prices_round_0_day_-1.csv"]:
    ...
    for alpha in [0.05, 0.1, 0.15, 0.2, 0.3, 0.5]:
        ...
        for thresh in [1, 2, 3, 4, 5]:
            ...
            print(
                f"  alpha={alpha:.2f}, thresh={thresh}: {opps} opportunities ({opps / len(deviations) * 100:.1f}%), avg_dev={avg_dev:.2f}"
            )
        print()
